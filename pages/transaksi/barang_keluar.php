...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Cek stok dulu
    $stok_check = mysqli_query($conn, "SELECT stok FROM barang WHERE id_barang=$id_barang");
    ...
    if ($stok_data && $stok_data['stok'] >= $jumlah) {
        ...
        if ($update) {
            ...
            mysqli_query($conn, "INSERT INTO transaksi (id_barang, id_user, jumlah, jenis, keterangan) VALUES ($id_barang, $id_user, $jumlah, 'keluar', '$keterangan')");
            $message = "<div class='alert alert-success'>Barang keluar berhasil dicatat.</div>";
        } else {
            $message = "<div class='alert alert-danger'>Terjadi kesalahan saat mencatat transaksi.</div>";
        }
    } else {
        $message = "<div class='alert alert-warning'>Stok barang tidak mencukupi.</div>";
    }
}
