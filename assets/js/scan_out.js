...
function startScanOut() {
    ...
    navigator.mediaDevices.getUserMedia({ video: { facingMode: "user" } })
        .then(stream => {
            ...
            video.play();
            message.innerHTML = `<div class='alert alert-success'>✅ Kamera aktif, arahkan ke barcode untuk mengurangi stok.</div>`;
            isScanningOut = true;
            initScannerOut();
        })
        .catch(err => {
...
function updateStok(kode, tipe) {
    ...
    .then(data => {
        if (data.success) {
            message.innerHTML = `<div class='alert alert-success'>✅ ${data.nama_barang} dikurangi. Stok baru: ${data.stok_baru}</div>`;
        } else {
